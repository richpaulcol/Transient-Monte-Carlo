...
#			self.Friction()
		...
		if self.Friction_Model == 'Modified_Brunone':
		...
		CpList = self.TranH[-1,:-2] + self.TranQ[-1,:-2]*(self.B- self.Rp[:-2] *abs(self.TranQ[-1,:-2])*(1-epsilon))
		CmList = self.TranH[-1,2:] - self.TranQ[-1,2:]*(self.B - self.Rm[2:] *abs(self.TranQ[-1,2:])*(1-epsilon))
