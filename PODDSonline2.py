...
beta = 0.2			#Initial Guess of mobilisation rate (1/s)
Sol1 = alpha/beta
Sol2 = alpha/beta**0.5

### Simulation Properties
n = 4.				#Resolution Tweak (-)
...
#Driving Functions (
IncomingTMass = np.ones(times.size)*1e-6#np.random.uniform(0,0.1,times.size)
...
alpha = 3.216			#Initial Guess of amount fo material per m ^2
beta = 0.3
...
Obs_Covariance = np.array([[1e-3,0],[0,2e-3]])
#Obs_Covariance = np.array([[1e-2,0,0],[0,1e-2,0],[0,0,1e-3]])

Trans_Variance = np.ones(States.shape[0])
Trans_Variance[:-5] = (dt**2)*1e-15	
Trans_Variance[-5] = (dt**2)*1e-15		#Transition Uncertainty in incomingTMass
Trans_Variance[-4] = (dt**2)*1e-1			#Transition Uncertainty in tau_a
Trans_Variance[-3] = (dt**2)*1e-10			#Transition Uncertainty in tau_s
Trans_Variance[-2] = (dt**2)*1e-4			#Transition Uncertainty in Alpha
Trans_Variance[-1] = (dt**2)*1e-4			#Transition Uncertainty in Beta
...
#		States[:,t],Covariances[:,:,t] = AUSKF.filter_update(States[:,t-1],Covariances[:,:,t-1],np.hstack((TMeasured[t],T2Measured[t],tau_Measured[t])).T)
	...
	Covariances[:,:,t] += np.diag(np.random.uniform(1e-6,1e-5,States.shape[0]))
